<template>
  <div>
    <header class="p-4 flex justify-between items-center w-full bg-black z-10">
      ...
      <div @click="showModal = !showModal" class="hidden sm:flex space-x-4">
        <div @click="toggleCart('cart1')" class="relative flex items-center justify-center w-16 h-18 text-center text-gray-400 hover:text-white space-x-1 cursor-pointer">
          ...
          <span v-if="cartCount > 0" class="relative -top-2 -left-1 bg-red-500 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">
              {{ cartCount }}
          </span>
    ...
    <main v-if="beat" class="bg-linear flex justify-center px-5 sm:px-10 md:px-28 my-8">     
      <div class="w-full max-w-4xl bg-black shadow-lg p-4 rounded-lg">
        <div class="flex flex-col md:flex-row justify-center items-center">
          <div class="w-full md:w-64 text-white mb-4">
            <img 
               :src="`${beat.capa}`" 
               alt="Beat Cover" 
               class="rounded-lg shadow-lg w-full object-cover opacity-40">
          </div>
          ...
          <div class="w-full md:flex-1 md:ml-6">
            <h2 class="text-xl font-bold text-gray-100">
              {{ beat.titulo }}
                ...
                <p class="text-sm text-gray-400 mt-1">by prod.jp</p>
            ...
            <div class="mt-4 cursor-pointer" @click="playerSong(beat.audio)">

              <div class="w-24 flex justify-center items-center mx-auto">
                <svg v-if="!isPlayer || isPlayer && audioAtual !== beat.audio" 
            ...
            <div class="rounded-lg p-1 h-24" v-if="isPlayer">
              <img 
                src="/audios/som.gif" 
                alt="GIF animado" 
                class="w-full rounded-lg mb-4 h-10"
              >
            </div>
            <div class="mt-2 mb-2">
              <div class="mt-4 bg-gray-500 opacity-70 rounded-lg p-4 flex justify-between items-center">
                <p class="text-gray-300">Valor da licença:</p>
                <p class="text-xl font-semibold">{{ beat.valor }}</p>
              </div>
            </div>

            <div class="flex items-center">
                <button @click="addToCart(beat)" class="flex gap-2 bg-blue-600 hover:bg-blue-500 text-white py-2 px-4 rounded-lg w-full sm:w-auto mt-12 cursor-pointer">
                  Compre agora
                  <svg xmlns="http://www.w3.org/2000/svg" 
...
<script setup>
  ...
  onMounted(() => {
    const id = router.params.beat;
    beat.value = beats.value.find(item => item.id.toString() === id.toString());
  })
